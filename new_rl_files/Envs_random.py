...
class rlEnvs(Env):
    ...
    def step(self, action: int):
        ...
        ## check for truncation
            ...
            self.truncated = True
        # elif True:
        theta = (action-7)*np.pi/4
        if self.noise:
            theta = theta +0*sqrt(2*self.d*self.delt)*np.random.normal(0,1)
        ...
        if x_new < self.start_x:
            ...
            x_new = self.target_x-0.0001
            self.reward-=5

        if y_new < self.start_x:
            y_new = self.start_x+0.0001
            self.reward-=5
        elif y_new > self.target_x:
            y_new = self.target_x-0.0001
            self.reward-=5
