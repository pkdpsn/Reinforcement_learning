...
def create_grid(conf=None):
    ...
    option = conf["options"]
    print(f"x_first: {x_first}, x_second: {x_second}, y_first: {y_first}, y_second: {y_second}, rows: {rows}, cols: {cols}, option: {option}")
    def potential1(x, y):
        # Mexican Hat
    ...
    y_values = np.linspace(y_second, y_first, rows)
    ##print x y in grid format
...
def print_grid_and_path(grid, states, conf, save_path=None, plotting = False, graph_title = None):
    ...
    plt.xticks(x_ticks, x_labels)
    y_ticks = np.linspace(0, rows-1, num_ticks)
    y_labels = np.linspace(y_second, y_first, num_ticks)
    plt.yticks(y_ticks, y_labels)
    plt.colorbar()
    ...
    ###plot start and end acc to the config file
    # plt.scatter(10,40, color='red',s=20 , marker = 'x')
    # plt.scatter(70,40, color='cyan',s=20 , marker = 'x')
